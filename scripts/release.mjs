...
const smoke = join(tmpdir(), `ds-smoke-${sh('git rev-parse --short HEAD').trim()}`);
...
for (const n of names) {
  if (!html.includes('${PREFIX}-' + n)) { console.error(n + ' rendered without its class'); process.exit(1); }
}

/*
 * The stylesheet has to carry tokens, and carry them before the rules that
 * resolve them. Asked structurally rather than by naming a token: a component
 * resolves var(--x) against whatever the cascade has already seen, so tokens
 * appearing after the first component rule is a file that silently renders
 * unstyled in a consumer's app.
 */
const firstToken = css.search(/^\s*--[a-z][a-z0-9-]*\s*:/m);
const firstComponent = css.indexOf('.${PREFIX}-');
if (firstToken === -1) { console.error('styles.css carries no tokens'); process.exit(1); }
if (firstComponent > -1 && firstToken > firstComponent) {
