...
import {
  ...
  semver, cmpSemver, STATUSES, PREFIX,
...
function reviewComponent(name) {
  ...
  /*
   * Raw px is allowed in exactly one place: the `--<prefix>-<Name>-unbound-*`
  ...
  for (const line of stripBlockComments(c.css ?? '').split('\n')) {
    ...
    if (new RegExp(`--${PREFIX}-[A-Za-z]+-unbound-`).test(line)) continue;
  ...
  if (c.hasClassPrefix) pass(`CSS class prefix matches — .${PREFIX}-${name}`);
  else if (c.css) fail(`no .${PREFIX}-${name} class in ${name}.css — the CSS names it something else`);
