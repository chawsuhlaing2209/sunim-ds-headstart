...
function storyEmbed(storybook, id, { view = 'story', kind = 'story', title } = {}) {
  ...
  return [
    '<div class="ref-embed-pair">',
    `<iframe class="ref-embed ref-embed--${kind}" data-embed-theme="light" src="${src('day')}" title="${title} — day" loading="lazy"></iframe>`,
    `<iframe class="ref-embed ref-embed--${kind}" data-embed-theme="dark" src="${src('night')}" title="${title} — night" loading="lazy"></iframe>`,
    '</div>',
  ].join('\n');
}
...
function componentPage(c, index) {
   ...
   * it", four lines apart.
   */
  push('<p class="ref-pageheader">');
  push(`<span class="ref-status">${intent.status} · since ${intent.since}</span>`);
  ...
  if (examples.length) {
    ...
    for (const s of examples) {
      ...
      push('');
      push(`<p class="ref-embed-note">Rendered live from Storybook · <a href="${storyLink(s.id)}">open it there</a> to change the props or the mode.</p>`);
      push('');
    }
  }
  push('</TabItem>');
  ...
  for (const dotted of intent.required_tokens) {
    ...
    const swatch = isColour(resolved) ? `<span class="ref-swatch" style="background:${resolved}"></span>` : '';
  ...
  if (kind) {
    ...
    push('');
    push(`<p class="ref-embed-note">If this frame is blank, the deployed Storybook is refusing to be `
  ...
  if (c.figma) {
    ...
    push('');
    push(`<iframe class="ref-embed ref-embed--figma" src="${c.figma.embed}" title="${c.name} in Figma" loading="lazy" allowfullscreen></iframe>`);
    push('');
    push(`<p class="ref-embed-note">The frame renders only for someone who can see the file. If it `
  ...
  push('');
  push('<div class="ref-modes">');
  for (const mode of modes) {
    push(`<div class="ref-mode" data-ref-mode="${mode}">`);
    push('<div class="ref-mode__swatches">'
      ...
      + '</div>');
    push(`<div class="ref-mode__name">${mode}</div>`);
    push('</div>');
  }
  push('</div>');
  push('');
 ...
 * `[data-ref-mode]` so a page can show them side by side.
 *
...
function tokenStylesheet() {
  ...
  const out = [
    ...
    ' * reachable under [data-ref-mode] for pages that show them side by side.',
    ' */',
    '',
    `:root {${root}\n}`,
    '',
  ...
  for (const mode of modes) {
    const body = blockBody(`[data-theme="${mode}"]`);
    if (!body) continue;
    out.push(`[data-ref-mode='${mode}'] {${body}\n}`);
    out.push('');
  }
