...
> **This table is a reading of the six agent files that ship with this repo, and it
> arrives already filled in for them.** That is deliberate: a blank table teaches
> nothing, and the six files are the same six you have. It is still a reading rather
> than a fact — the moment you edit an agent file, add a seventh agent, or grant one
> a column it did not have, the row describing it is stale and it is yours to
> re-derive. The method is in *Why each type and level* below: three questions about
> the machine, then what its own file lets it reach.
>
> Nothing about your project is in here yet. Who reviews, who can stop the fleet, and
> what happens when something breaks live in [`operating-model.md`](operating-model.md),
> which ships blank on purpose.
...
and the orchestrator can name two blind spots and how they handle them — beyond the
...
orchestrator can name two blind spots and how they handle them.
...
**not stated** anywhere in the repo. Check your remote's branch protection settings and
