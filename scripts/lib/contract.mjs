...
/*
 * The repo root, resolved from this file rather than from the working directory.
 *
 * `generate-docs.mjs` runs with cwd set to `docs/` and chdirs to the root in its
 * body — but an ESM import is evaluated before the importing module's body runs,
 * so anything this module reads at load time would read the wrong package.json.
 * Deriving it from the module's own location makes that ordering irrelevant.
 */
...
/*
 * The component class and custom-property prefix — `.ds-Button`,
 * `--ds-Button-unbound-size`.
 *
 * Read from `package.json` rather than written here, because three separate
 * gates check it and a prefix that lives in three files is a prefix that will
 * disagree with itself. A repo cloned to start a new system changes one field.
 */
export const PREFIX = (() => {
  const p = readPackage().designSystem?.prefix;
  if (!p) {
    throw new Error(
      'package.json has no designSystem.prefix. Every component class is built from it '
      + '(.<prefix>-Button), and the release gates check for it. Set it before building anything.',
    );
  }
  if (!/^[a-z][a-z0-9]*$/.test(p)) {
    throw new Error(`designSystem.prefix must be lowercase alphanumeric — got "${p}".`);
  }
  return p;
})();
...
export function readComponent(name) {
  ...
  return {
    ...
    hasClassPrefix: Boolean(css && css.includes(`${PREFIX}-${name}`)),
  };
}
...
export function parseFigma(source) {
  ...
  return {
     ...
     * browser.
     */
    embed: `https://embed.figma.com/design/${fileKey}/${slug}?node-id=${rawNode.replace(':', '-')}&embed-host=${PREFIX}-reference`,
  };
}
...
export function parseUnbound(css) {
  return [...stripBlockComments(css).matchAll(
    new RegExp(`(--${PREFIX}-[A-Za-z]+-unbound-[a-z0-9-]+)\\s*:\\s*([^;]+);`, 'gi'))]
  ...
  if (!required.length) {
    ...
    const referenced = new Set(
      [...`${css ?? ''}`.matchAll(/var\(\s*(--[a-z0-9-]+)/gi)]
        .map((m) => m[1])
        .filter((v) => !v.startsWith(`--${PREFIX}-`) && tokens.has(v)),
    );
