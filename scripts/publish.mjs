 ...
 *
 * Two paths exist and both are complete. Which one is right for you depends on
 * something outside this repo: whether your account can run GitHub Actions.
 *
 *   CI — `.github/workflows/release-publish.yml`, triggered by hand. Publishes
 *   from a clean checkout with `--provenance`: a signed attestation binding the
 *   tarball to the commit and workflow that built it, which anyone can verify
 *   without trusting you. Needs the `NPM_TOKEN` repository secret. Prefer this
 *   when it works.
 *
 *   This script — publishes from your machine. It runs the same nine gates and
 *   the same smoke install, and cannot attach provenance: that needs the OIDC
 *   token a CI runner has and a laptop does not.
 *
 * If Actions cannot run for you — a locked account, no billing method, an
 * offline setup — this path is not a downgrade in rigour. Every gate still runs.
 * What you lose is the attestation, and step 8 records what it can in its place:
 * the commit, the tag, and the registry's own checksum for the tarball, appended
 * to the release report. That is a claim a person can re-check, not one a
 * machine can verify, and a release note may say the publish was *recorded* but
 * never that it was *attested*.
 *
 * What this publishes is trusted because you ran it. Nothing else.
 *
 * What it will not do:
 *
...
const smoke = `/tmp/ds-verify-${pkg.version}`;
try {
