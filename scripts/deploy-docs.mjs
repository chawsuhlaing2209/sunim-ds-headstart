 ...
 */
const PROJECT_NAME = JSON.parse(readFileSync('docs/reference.config.json', 'utf8')).vercelProject;
if (!PROJECT_NAME) {
  stop('docs/reference.config.json has no vercelProject.',
    'Name the Vercel project the reference site deploys to. It is not a credential —\n'
    + '  it is the same name that appears in the site\'s own domain.');
}
